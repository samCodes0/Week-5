...
rates[Symbol('CHF')] = 1.1787
// TODO if you had 100 Euros, write code to get the exchange rate from the object, then calculate
//      the equivalent value in Australian Dollars (AUD)
function exchangeEuros(euros, currency) {
  return euros * rates[currency];
}

console.log(exchangeEuros(100, 'AUD'));
...
cats_and_owners.forEach( function(entry) {
  console.log(`${entry.name}'s cat is named ${entry.cat}`);
})
...
for (let i = 0; i < cats_and_owners.length; i ++){
  let entry = cats_and_owners[i];
  if (entry.name === "Gary Oldman") {
    console.log(entry.cat)
    break;
  }
}
...
let fullName = `${literatureNobelLaureate.firstname} ${literatureNobelLaureate.surname}`;
console.log(fullName);
// TODO print the ids of each of the Physics Nobel laureates. Your code should still work without modification if a laureate was added, or removed.
let physicsNobelLaureats = nobel_prize_winners_2017.prizes[0].laureates;
for (laureate in physicsNobelLaureats) {
  console.log(physicsNobelLaureats[laureate].id);
}
...
nobel_prize_winners_2017.prizes.forEach((prize) => {
  if (prize.year === "2017"){
    numLaureates += prize.laureates.length;
  }
})
console.log(numLaureates);
//   have a good look at how the JSON is structured, and think about what loop(s) you'll need to write.
// I would probably have to loop over all the prizes categories to start with
// if necessary I would have to dig into the laureates of each one of the prize categories